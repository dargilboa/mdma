...
def estimate_skeleton(model, data, alpha, **kwargs):
  ...
  if 'init_graph' in kwargs:
    ...
    if not isinstance(g, nx.Graph):
      ...
      raise ValueError('init_graph not matching model.d')
    for (i, j) in combinations(node_ids, 2):
      ...
      if len(adj_i) >= l:
        ...
        for k in combinations(adj_i, l):
          _logger.debug('indep prob of %s and %s with subset %s' %
                        (i, j, str(k)))
          p_val = ci_test(model, data, i, j, set(k))[1][0]
...
def estimate_cpdag(skel_graph, sep_set):
  ...
  def _has_one_edge(dag, i, j):
    return ((dag.has_edge(i, j) and (not dag.has_edge(j, i)))
            or (not dag.has_edge(i, j)) and dag.has_edge(j, i))
