...
def hessian_penalty(G,
  ...
  loss = multi_stack_var_and_reduce(
      second_orders, reduction,
      return_separately)  # (k, G(z).size()) --> scalar
  return loss
...
def multi_layer_second_directional_derivative(G, z, x, G_z, epsilon,
  ...
  eps_sqr = epsilon**2
  sdd = [(G2x - 2 * G_z_base + Gfx) / eps_sqr
...
def multi_stack_var_and_reduce(sdds,
                               reduction=torch.max,
                               return_separately=False):
...
def _test_hessian_penalty():
  ...
  print('Approximation: %s' % predicted
